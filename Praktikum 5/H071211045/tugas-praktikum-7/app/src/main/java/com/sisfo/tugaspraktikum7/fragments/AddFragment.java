...
public class AddFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.btnUpload.setOnClickListener(v -> {
            ...
            Post post = new Post("Hj. Muhammad Soeharto", "mr_soeharto", imageUri, binding.etCaption.getText().toString());
    ...
    private final ActivityResultLauncher<Intent> galleryLauncher = registerForActivityResult(
            new ActivityResultContracts.StartActivityForResult(), photo -> {
                if (photo.getResultCode() == Activity.RESULT_OK) {
                    imageUri = photo.getData().getData();
                    binding.ivPostImageView.setImageURI(imageUri);
                }
            });
}