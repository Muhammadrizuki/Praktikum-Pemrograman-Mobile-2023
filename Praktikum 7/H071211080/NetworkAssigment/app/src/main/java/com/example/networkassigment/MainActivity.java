...
public class MainActivity extends AppCompatActivity {
    ...
    public static ArrayList<User> dataPerson = new ArrayList<>();
    ...
    private void getDataApi() {
        if (isNetworkAvailable()) {
            ...
            Call<Data> client = ApiConfig.getApiService().getUser("12");
            client.enqueue(new Callback<Data>() {
                @Override
                public void onResponse(Call<Data> call, Response<Data> response) {
                    if (response.isSuccessful()) {
                        if (response.body() != null) {
                            List<User> userResponse = response.body().getData();
                            Adapterv adapter = new Adapterv(MainActivity.this, userResponse);
